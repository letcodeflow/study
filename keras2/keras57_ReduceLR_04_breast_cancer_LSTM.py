...
reduce_lr = ReduceLROnPlateau(monitor='val_loss',mode='min',verbose=1,factor=0.0005,patience=14)
ES = EarlyStopping(monitor='val_loss', patience=90, mode='min', verbose=1, restore_best_weights=True)
g = model.fit(x_train, y_train, epochs=1, batch_size=10, validation_split=0.1, callbacks=[ES,reduce_lr], verbose=2)
