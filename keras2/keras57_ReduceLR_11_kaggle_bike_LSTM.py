...
reduce_lr = ReduceLROnPlateau(monitor='val_loss',mode='min',patience=14,factor=0.0005,verbose=1)
import datetime as dt
date = dt.datetime.now()
date=date.strftime('%m%d %H%M')
filename = '{epoch:04d} {val_loss:.4f}.hdf5'
filepath = './_ModelCheckPoint/k25/10/'
# mcp = ModelCheckpoint('dkjfw.h5',monitor='val_loss', mode='auto', save_best_only=True)
...
model.fit(x_train, y_train, epochs=25, batch_size=200, verbose=1, validation_split=0.2, callbacks=[reduce_lr])
