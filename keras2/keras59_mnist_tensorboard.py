...
tb = TensorBoard(log_dir='C:/Users/aiapalm/OneDrive - KNOU/beat/study/_data/tensorboard_log/_graph',histogram_freq=0,write_graph=True,write_images=True)
# cmd-해당폴더 이동 = tensorboard --logdir=.
# ip주소:6006 접속도 동일

es = EarlyStopping(monitor='val_loss',mode='min',patience=20,verbose=1)
reduce_lr = ReduceLROnPlateau(monitor='val_loss',mode='auto',patience=10,factor=0.00005,verbose=1)

# import tensorflow.python.keras.optimizers import adamax
# from tf.keras.optimizers.Optimizer
...
import time
start = time.time()
hist = model.fit(x_train,y_train,epochs=50,batch_size=32,callbacks=[es,reduce_lr,tb],validation_split=0.2,verbose=1)
end = time.time()-start
