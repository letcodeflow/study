...
# value count  쓰지않고 groupby count 써서 그래프
print(dataset.groupby('quality').count())
plt.bar(height = dataset.groupby('quality')['quality'].count().index,dataset.groupby('quality')['quality'].count())
plt.show()
...
x_train, x_test, y_train, y_test = train_test_split(x,dataset[:,-1],random_state=23786,train_size=0.8, stratify=dataset[:,-1])
