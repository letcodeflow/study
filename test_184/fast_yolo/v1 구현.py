#(kernel_size, output_channel, strides, padding)
arch = [
    (7, 64, 2, 3),
    'Pool',
    (3, 192, 1, 1),
...
class Yolov1(nn.Module):
    def __init__(self, in_channels=3, **kwargs):
        ...
        self.darkent = self.darknet_conv_net(self.architecture)
    ...
    def darknet_conv_net(self, architecture):
        ...
        for x in architecture:
            if type(x) == tuple:
                layers += [
                    CNNBlock(in_channels, x[1], kernerl_size=x[0], stride=x[2], padding=x[3])
                ]
                in_channels = x[1]
            elif type == str:
